...
export async function verifyBotProtection({
  ...
  try {
    ...
    const clientIp =
      headerList.get("x-forwarded-for") || headerList.get("cf-connecting-ip");

    const turnstileSecret =
      env.TURNSTILE_SECRET_KEY || process.env.TURNSTILE_SECRET_KEY;
    const token =
      turnstileToken ||
      headerList.get("x-turnstile-token") ||
      headerList.get("cf-turnstile-response");
...
export async function getClientIp(): Promise<string> {
  try {
    ...
    const realIp =
      headerList.get("x-real-ip") || headerList.get("cf-connecting-ip");
    if (realIp) {
      return realIp.trim();
    }
  } catch {
