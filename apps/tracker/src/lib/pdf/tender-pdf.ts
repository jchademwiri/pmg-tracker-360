...
function drawMeta(
  ...
  doc.setFontSize(9);
  doc.setTextColor(24, 24, 27);
  doc.text(formatDate(data.submissionDate), PAGE.width - PAGE.margin, y, {
    align: "right",
  });
  ...
  doc.setFontSize(9);
  doc.setTextColor(24, 24, 27);
  doc.text(formatDate(data.validityDate), PAGE.width - PAGE.margin, y + 7, {
    align: "right",
  });
  ...
  doc.setFontSize(9);
  doc.setTextColor(24, 24, 27);
  doc.text(formatCurrency(data.value), PAGE.width - PAGE.margin, y + 14, {
    align: "right",
  });
  ...
  if (data.awardValue) {
    ...
    doc.setTextColor(24, 24, 27);
    doc.text(
      formatCurrency(data.awardValue),
...
function renderPdf(data: {
  ...
  if (data.status === "lost" && (data.lossReason || data.lossDetails)) {
    ...
    const detailLines = data.lossDetails
      ...
      : [];
    const neededHeight =
      10 + (data.lossDetails ? detailLines.length * 4 + 8 : 0);
