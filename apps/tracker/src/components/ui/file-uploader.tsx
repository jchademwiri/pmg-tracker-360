...
function getFileColor(fileName: string) {
  const ext = fileName.split('.').pop()?.toLowerCase() || '';
  if (['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(ext)) return 'text-purple-600 bg-purple-500/10 border-purple-500/20';
  if (['xls', 'xlsx', 'csv'].includes(ext)) return 'text-emerald-600 bg-emerald-500/10 border-emerald-500/20';
  if (['pdf'].includes(ext)) return 'text-rose-600 bg-rose-500/10 border-rose-500/20';
  if (['doc', 'docx'].includes(ext)) return 'text-sky-600 bg-sky-500/10 border-sky-500/20';
    ...
    (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
      ...
      if (onValueChange && acceptedFiles.length > 0) {
        ...
        if (currentFiles.length + newFiles.length > maxFiles) {
          toast.warning(`Maximum ${maxFiles} file${maxFiles > 1 ? 's' : ''} allowed`);
        }
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            PDF, Word, Excel, Images (max {Math.round(maxSize / 1024 / 1024)}MB per file, up to {maxFiles} {maxFiles === 1 ? 'file' : 'files'})
          </p>
        </div>
      </div>

      {value.length > 0 && (
        <ScrollArea className={cn(value.length > 3 ? 'h-[200px]' : 'h-auto', 'w-full rounded-lg border border-border/60 p-2.5 bg-background/50')}>
              ...
              return (
                ...
                >
                  <div className="flex items-center space-x-3 overflow-hidden min-w-0 flex-1">
                    <div className={cn('p-2 rounded-md border shrink-0', colorClass)}>
