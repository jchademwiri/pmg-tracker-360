  ...
  return (
    <div className={`space-y-1.5 leading-relaxed break-words ${className}`}>
      {lines.map((line, lineIdx) => {
        ...
        // Format tokens: URLs, @mentions, `code`, **bold**
...
function parseLine(text: string): React.ReactNode[] {
  // Regex to match URLs, @mentions, `code`, **bold**
  const pattern =
    ...
    if (match[1]) {
      nodes.push(
        <a
          ...
          className="underline text-indigo-400 hover:text-indigo-300 font-medium break-all inline-flex items-center gap-0.5"
