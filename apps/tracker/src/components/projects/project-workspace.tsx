...
export function ProjectWorkspace({
  ...
  const [activities, setActivities] =
    useState<ProjectActivityType[]>(initialActivities);
  ...
  const [selectedRisk, setSelectedRisk] = useState<ProjectRiskType | null>(
    null,
  );
  ...
  const allPOsDelivered =
    purchaseOrders.length === 0 ||
    purchaseOrders.every((po) =>
      ["delivered", "completed"].includes(po.status),
    );
  const allRisksClosed =
    risks.length === 0 || risks.every((r) => r.status === "closed");
  const hasSignedContract =
    !!project.signedContractUrl ||
    documents.some(
      (doc) =>
        doc.name.toLowerCase().includes("contract") ||
        doc.name.toLowerCase().includes("sla"),
    );
  const allDeliveryNotesVerified =
    deliveryNotes.length === 0 ||
    deliveryNotes.every((dn) => dn.status === "verified");
    ...
    startUpdateRiskTransition(async () => {
      try {
        ...
        if (res.success && res.risk) {
          ...
          setRisks((prev) =>
            prev.map((r) => (r.id === selectedRisk.id ? res.risk! : r)),
          );
      ...
      <div className="rounded-xl border bg-card p-6 md:p-8 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground max-w-2xl font-light">
                {project.description ||
                  "Workspace delivery dashboard for tracking purchase orders, documents, activity logs, and delivery risks."}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0">
            <Link
              href={`/projects/purchase-orders/create?projectId=${project.id}`}
            >
              <Button>
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              {project.status !== "completed" ? (
                  ...
                  <DialogContent className="sm:max-w-[500px]">
                    <DialogHeader>
                      <DialogTitle className="text-xl font-bold">
                        Project Close-out Submission
                      </DialogTitle>
                      <DialogDescription className="text-muted-foreground">
                        Archive the project as completed. This records close-out
                        notes, captures the date, and changes the project
                        status.
                      </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                      ...
                      <div className="rounded-lg border bg-muted/20 p-4 space-y-3">
                        ...
                        <div className="space-y-2.5">
                          {closeOutChecklist.map((item) => (
                            ...
                            >
                              ...
                              <div>
                                <p
                                  className={`font-medium ${item.status ? "text-foreground" : "text-muted-foreground"}`}
                                >
                                  {item.label}
                                </p>
                        ...
                        {criticalRisks.length > 0 && (
                          <div className="mt-3 p-2.5 rounded-lg bg-red-500/10 border border-red-500/30 text-xs text-red-600 dark:text-red-400 flex items-start gap-2">
                            ...
                            <span>
                              <strong>Critical risks must be resolved</strong>{" "}
                              before close-out can be submitted. Please mitigate{" "}
                              {criticalRisks.length > 1
                                ? "these risks"
                                : "this risk"}{" "}
                              in the Risks tab first.
                            </span>
                          </div>
                        )}
                        {!closeOutChecklist.every((item) => item.status) &&
                          criticalRisks.length === 0 && (
                            <div className="mt-3 p-2.5 rounded-lg bg-amber-500/5 border border-amber-500/20 text-xs text-amber-600 dark:text-amber-400 flex items-start gap-2">
                              ...
                              <span>
                                Some checks are not satisfied. You can still
                                submit the close-out if necessary.
                              </span>
                            </div>
                          )}
                      </div>
                      <div className="space-y-2">
                        ...
                        />
                        <p className="text-xs text-muted-foreground">
                          Min. 10 characters required. Logs submitter name and
                          timestamp.
                        </p>
                      </div>
                    </div>
                ...
                <Button
                  variant="ghost"
                  disabled
                  className="text-muted-foreground bg-muted/50 border border-border/40 rounded-xl"
                >
          ...
          {[
            ...
            return (
              <button
                ...
                className={`group inline-flex h-9 shrink-0 items-center gap-2 rounded-md px-3 text-sm font-medium transition-all duration-200 cursor-pointer ${
                  isActive
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription className="text-muted-foreground text-xs">
                        Overall completion based on Purchase Order fulfillment
                      </CardDescription>
                    </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-4 pt-2">
                    ...
                    <div className="p-3 bg-muted/50 rounded-lg border border-border/30">
                      ...
                      <p className="text-sm font-semibold text-amber-600 dark:text-amber-500 mt-1">
                        {formatCurrency(
                          Math.max(0, totalPOAmount - totalDeliveredValue),
                        )}
                      </p>
                    </div>
                  </div>
              ...
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription className="text-muted-foreground text-xs">
                    Active delivery vulnerabilities
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between py-2 border-b border-border/30">
                    <div className="flex items-center space-x-2">
                      <div
                        className={`h-2.5 w-2.5 rounded-full ${criticalRisks.length > 0 ? "bg-red-500 animate-pulse" : "bg-muted-foreground"}`}
                      />
                    ...
                    <span
                      className={`text-sm font-bold ${criticalRisks.length > 0 ? "text-red-600 dark:text-red-400" : "text-muted-foreground"}`}
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription className="text-muted-foreground text-xs">
                    Origin tender details for context
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {project.tender ? (
                    <>
                      ...
                      {project.tender.description && (
                        <div>
                          ...
                          <p className="text-sm text-foreground mt-0.5 font-light line-clamp-3">
                            {project.tender.description}
                          </p>
                        </div>
                      )}
                      ...
                      {project.tender.value && (
                        <div>
                          ...
                          <p className="text-sm font-semibold text-foreground mt-0.5">
                            {formatCurrency(project.tender.value)}
                          </p>
                        </div>
                      )}
                      ...
                      {project.tender.submissionDate && (
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-0.5">
                            {formatDate(project.tender.submissionDate)}
                          </p>
                        </div>
                      )}
                    </>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription className="text-muted-foreground text-xs">
                    Dates, values, and closeout archives
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription className="text-muted-foreground text-xs">
                    A list of purchase orders issued and their delivery states
                  </CardDescription>
                </div>
                <Link
                  href={`/projects/purchase-orders/create?projectId=${project.id}`}
                ...
                {purchaseOrders.length > 0 ? (
                  ...
                  <div className="text-center py-12 text-muted-foreground border border-dashed border-border/40 rounded-xl">
                    ...
                    <p className="text-sm font-light">
                      No purchase orders linked to this project workspace.
                    </p>
                    <Link
                      href={`/projects/purchase-orders/create?projectId=${project.id}`}
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription className="text-muted-foreground text-xs">
                    Saved line items available for purchase orders on this
                    project
                  </CardDescription>
                </div>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription className="text-muted-foreground text-xs">
                    Delivery notes recorded against purchase orders in this
                    project
                  </CardDescription>
                </div>
              ...
              <CardContent>
                {deliveryNotes.length > 0 ? (
                  <div className="space-y-4">
                    {deliveryNotes.map((note) => {
                      const noteValue = (note.items || []).reduce(
                        (sum, item) =>
                          sum + parseFloat(item.deliveryValue || "0"),
                        0,
                      );
                      ...
                      return (
                        ...
                        >
                          <div className="space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground">
                              PO {note.poNumber} • received by{" "}
                              {note.recipientName} on{" "}
                          ...
                          <div className="flex items-center justify-between gap-4 md:justify-end">
                            ...
                            <Link
                              href={`/projects/purchase-orders/${note.poId}`}
                            >
                              <Button
                                variant="ghost"
                                size="icon"
                  ...
                  <div className="text-center py-12 text-muted-foreground border border-dashed border-border/40 rounded-xl">
                    ...
                    <p className="text-sm font-light">
                      No delivery notes recorded for this project yet.
                    </p>
                    {purchaseOrders.length > 0 && (
          ...
          <TabsContent value="activity" className="space-y-6 outline-none">
            <Card>
              <CardHeader>
                ...
                <CardDescription className="text-muted-foreground text-xs">
                  Chronological timeline of all workspace lifecycle events
                </CardDescription>
              </CardHeader>
              <CardContent className="px-6 pb-8">
                {activities.length > 0 ? (
                  ...
                  <div className="text-center py-12 text-muted-foreground border border-dashed border-border/40 rounded-xl">
                    ...
                    <p className="text-sm font-light">
                      No logged activities for this project.
                    </p>
                  </div>
                )}
              </CardContent>
          ...
          <TabsContent value="risks" className="space-y-6 outline-none">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  <CardTitle className="text-base font-semibold">
                    Delivery Risk Logging
                  </CardTitle>
                  <CardDescription className="text-muted-foreground text-xs">
                    Manage project constraints, severity logs, and mitigation
                    measures
                  </CardDescription>
                </div>
                ...
                >
                  ...
                  <DialogContent className="sm:max-w-[500px]">
                    <DialogHeader>
                      <DialogTitle className="text-xl font-bold">
                        Log Project Delivery Risk
                      </DialogTitle>
                      <DialogDescription className="text-muted-foreground">
                        Record a delivery vulnerability and assign its severity
                        to track mitigation progress.
                      </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
              ...
              <CardContent>
                {risks.length > 0 ? (
                  <div className="space-y-6">
                    ...
                    {/* Mitigate Dialog */}
                    <Dialog
                      open={isMitigateDialogOpen}
                      onOpenChange={setIsMitigateDialogOpen}
                    >
                      {" "}
                      <DialogContent className="sm:max-w-[500px]">
                        <DialogHeader>
                          ...
                          <DialogDescription className="text-muted-foreground">
                            Update the risk mitigation action plan or mark the
                            risk as mitigated/closed.
                          </DialogDescription>
                        </DialogHeader>
                        {selectedRisk && (
                          <div className="space-y-4 py-4">
                  ...
                  <div className="text-center py-12 text-muted-foreground border border-dashed border-border/40 rounded-xl">
                    ...
                    <p className="text-sm font-light">
                      No risks logged for this project workspace.
                    </p>
                    <Button
                      size="sm"
                      variant="outline"
