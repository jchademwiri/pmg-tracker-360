...
export function TenderActionQueue({
  organizationId,
  initialQueues,
}: TenderActionQueueProps) {
  ...
  const [activeTender, setActiveTender] = useState<ActionQueueTender | null>(
    null,
  );
    ...
    startTransition(async () => {
      try {
        ...
        if (result.success) {
          ...
          setQueues((prev) => ({
            ...prev,
            briefingPending: prev.briefingPending.filter(
              (t) => t.id !== tenderId,
            ),
          }));
          router.refresh();
    ...
    startTransition(async () => {
      try {
        ...
        if (result.success) {
          ...
          setQueues((prev) => ({
            ...prev,
            awardedToConvert: prev.awardedToConvert.filter(
              (t) => t.id !== activeTender.id,
            ),
          }));
  ...
  > = {
    ...
    briefingPending: {
      ...
      description:
        "Pre-submission tenders with mandatory briefings not yet recorded as attended.",
      icon: Calendar,
      colorClass:
        "text-blue-500 border-blue-500/20 bg-blue-500/5 hover:bg-blue-500/10 hover:border-blue-500/30",
    ...
    awaitingResults: {
      ...
      icon: ClipboardCheck,
      colorClass:
        "text-indigo-500 border-indigo-500/20 bg-indigo-500/5 hover:bg-indigo-500/10 hover:border-indigo-500/30",
      badgeVariant: "outline",
    },
    awardedToConvert: {
      label: "Awarded to Convert",
      description:
        "Tenders won/appointed that have not yet been converted into projects.",
      icon: Trophy,
      colorClass:
              ...
              <Table className="w-full table-fixed">
                ...
                <TableBody>
                  {paginationData.paginatedItems.map((t) => {
                    ...
                    const isTenderPending =
                      isPending && pendingTenderId === t.id;
                          ...
                          <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="flex flex-col gap-0.5 min-w-0">
                              <div className="font-bold text-foreground text-xs truncate tracking-tight">
                                {formatClientName(t.client?.name) ||
                                  "Unknown Client"}
                              </div>
                              <Link
                                href={`/tenders/${t.id}`}
                          ...
                          {selectedQueue === "briefingPending" && (
                            <div className="flex flex-col text-xs gap-0.5">
                              <div className="font-semibold text-foreground flex items-center gap-1.5">
                                <Calendar className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                                {t.briefingDate
                                  ? formatDate(t.briefingDate)
                                  : "—"}
                              </div>
                              {t.briefingLocation && (
                                <div
                                  className="text-[11px] text-muted-foreground pl-5 truncate max-w-[140px]"
                                  title={t.briefingLocation}
                                >
                                  {t.briefingLocation}
                                </div>
                              )}
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  className="size-8 cursor-pointer hover:bg-accent"
                                >
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t border-border bg-muted/10 text-xs text-muted-foreground">
                  <div>
                    ...
                    <span className="font-semibold text-foreground">
                      {paginationData.startItem}
                    </span>{" "}
                    to{" "}
                    <span className="font-semibold text-foreground">
                      {paginationData.endItem}
                    </span>{" "}
                    of{" "}
                    <span className="font-semibold text-foreground">
                  ...
                  {paginationData.totalPages > 1 && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium px-2">
                        Page {paginationData.safePage} of{" "}
