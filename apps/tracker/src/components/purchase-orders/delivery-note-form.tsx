...
export function DeliveryNoteForm({
    ...
    () =>
      po.lineItems.map((item) => {
        ...
        const previouslyDelivered = po.deliveryNotes.reduce((sum, note) => {
          const deliveredItem = note.items?.find(
            (di) => di.lineItemId === item.id,
          );
          return (
            sum +
            (deliveredItem
              ? parseFloat(deliveredItem.quantityDelivered) || 0
              : 0)
          );
        }, 0);
        const outstandingQuantity = Math.max(
          0,
          orderedQuantity - previouslyDelivered,
        );
        const deliveredQuantity =
          parseFloat(deliveredQuantities[item.id] || "0") || 0;
  ...
  const totalDeliveredQuantity = itemRows.reduce(
    (sum, item) => sum + item.deliveredQuantity,
    0,
  );
  const totalDeliveryValue = itemRows.reduce(
      ...
      try {
        ...
        if (uploadFiles.length > 0) {
          ...
          if (!uploadResult.success || !uploadResult.document) {
            toast.error(
              uploadResult.error || "Failed to upload Proof of Delivery file",
            );
            return;
          }
        ...
        toast.error(
          "An unexpected error occurred while saving the delivery note",
        );
      }
    });
  };
                    ...
                    <Card key={item.id} className="p-4 space-y-3 border">
                      ...
                      <div className="grid grid-cols-3 gap-2 text-xs text-muted-foreground text-center">
                        ...
                        <div className="bg-emerald-500/5 p-1.5 rounded">
                          <span className="block text-emerald-600/70">
                            Delivered
                          </span>
                          <strong className="block text-sm text-emerald-600 mt-0.5">
                            {item.previouslyDelivered}
                          </strong>
                        </div>
                        <div className="bg-muted/10 p-1.5 rounded">
