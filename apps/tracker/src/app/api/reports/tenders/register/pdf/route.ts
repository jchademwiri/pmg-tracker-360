...
export async function GET(request: Request) {
  ...
  if (!session.session?.activeOrganizationId) return NextResponse.json({ error: 'No organization selected.' }, { status: 400 });
  ...
  const result = await getTenderRegisterPdf(session.session.activeOrganizationId, clientId);
