...
import { getTenderFollowUpPdf } from "@/server/tender-follow-up-pdf";
...
export async function GET(request: Request) {
  ...
  try {
    const result = await getTenderFollowUpPdf(
    ...
    console.error("Tender follow-up PDF export failed:", error);
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "An unexpected error occurred generating follow-up PDF report.",
      },
      { status: 500 },
    );
  }
