 ...
 */
export async function getTenderCalendarEvents(
  ...
  try {
    ...
    const horizon = new Date(
      now.getTime() +
        (daysAhead || URGENCY_WINDOWS.UPCOMING_DEADLINES_DAYS) *
          24 *
          60 *
          60 *
          1000,
    ...
    for (const t of closingSoon) {
      ...
      const daysUntil = Math.ceil(
        (t.submissionDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
      );
      events.push({
        id: `close-${t.id}`,
        type: "closing_date",
    ...
    for (const t of validityTenders) {
      ...
      const daysUntil = Math.ceil(
        (t.evaluationDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
      );
      events.push({
        id: `validity-${t.id}`,
    ...
    const followUps = await db
      ...
      .orderBy(
        sql`COALESCE(${tenderFollowUp.nextFollowUpDate}, ${tenderFollowUp.followUpDate})`,
      );
    ...
    for (const e of extensions) {
      const daysUntil = Math.ceil(
        (e.newEvaluationDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
      );
      events.push({
        id: `ext-${e.id}`,
