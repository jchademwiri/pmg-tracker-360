...
const statusLabel = (status: string) =>
  getStatusConfig(status as never, "tender").label;
...
export async function getTendersExportExcel(
  ...
  try {
    ...
    rows.sort((a, b) => {
      const aTime = a.submissionDate
        ? new Date(a.submissionDate).getTime()
        : -Infinity;
      const bTime = b.submissionDate
    ...
    const clients = [
      ...new Map(
        rows
          .filter((r) => r.clientId && r.clientName)
          .map((r) => [r.clientId!, r.clientName!]),
      ).entries(),
    ].sort((a, b) => a[1].localeCompare(b[1]));
    if (clientId) {
      buildClientReport(
        workbook,
        rows[0].clientName || "Client",
        rows,
        orgName,
      );
    } else {
      ...
      clients.forEach(([id, name]) =>
        clientSheetNames.set(id, safeSheetName(name, usedSheetNames)),
      );
      ...
      for (const [id, name] of clients)
        buildClientSheet(
          workbook,
          name,
          rows.filter((r) => r.clientId === id),
          clientSheetNames.get(id)!,
          orgName,
        );
    }
...
function buildTenderRegister(
  workbook: ExcelJS.Workbook,
  rows: TenderExportRow[],
  orgName: string,
) {
  ...
  addTitle(
    ...
    "A1:L1",
    "TENDER REGISTER",
    `${orgName} · Master tender register — submission timing and data-quality checks update when the workbook is opened.`,
  );
  const headers = [
    "Tender Number",
    "Client",
  ...
  const tableRows = rows.map((row) => {
    const contact = [row.tenderContactName, row.clientEmail, row.clientPhone]
      .filter(Boolean)
      .map(cleanText)
      .join(" ");
    return [
      cleanText(row.tenderNumber).toUpperCase(),
      cleanText(row.clientName),
      cleanText(row.description),
      statusLabel(row.status),
      row.priority,
      dateValue(row.submissionDate),
      dateValue(row.briefingDate),
      row.value == null ? null : Number(row.value),
      row.awardValue == null ? null : Number(row.awardValue),
      contact,
      {
        formula:
          'IF([@[Submission Date]]="","Not Yet Due",IF([@[Submission Date]]<=TODAY(),"Submitted","Not Yet Due"))',
      },
      {
        formula:
          'IF([@[Contact Details]]="","Missing contact; ","")&IF([@[Estimated Value]]="","Missing estimate; ","")&IF(AND(ISNUMBER([@[Estimated Value]]),[@[Estimated Value]]<=1),"Questionable estimate","")',
      },
    ];
  });
  sheet.addTable({
    name: "TenderRegister",
    ref: `A${headerRow}:L${Math.max(headerRow + rows.length, headerRow + 1)}`,
    headerRow: true,
  ...
  [22, 30, 58, 18, 13, 18, 18, 18, 18, 42, 18, 34].forEach(
...
function buildContents(
  ...
  clientSheetNames: Map<string, string>,
  orgName: string,
) {
  ...
  addTitle(
    sheet,
    "A1:E1",
    "TENDER SUBMISSION SUMMARY",
    `${orgName} · Workbook navigation and live client-level tender totals.`,
  );
  [24, 34, 16, 16, 16].forEach(
    (width, i) => (sheet.getColumn(i + 1).width = width),
  );
  addKpi(
    sheet,
    ...
    "A5",
    "Total Registered",
    "COUNTA(TenderRegister[Tender Number])",
  );
  addKpi(
    sheet,
    "B4",
    "B5",
    "Total Submitted",
    'COUNTIF(TenderRegister[Submission Timing],"Submitted")',
  );
  addKpi(
    sheet,
    "C4",
    "C5",
    "Not Yet Due",
    'COUNTIF(TenderRegister[Submission Timing],"Not Yet Due")',
  );
  addKpi(
    sheet,
    "D4",
    "D5",
    "Number of Clients",
    `COUNTA(B13:B${12 + clients.length})`,
  );
  ...
  sheet.getRow(9).values = [
    "Summary",
    { text: "Open Summary", hyperlink: "#'Summary'!A1" },
  ];
  sheet.getRow(10).values = [
    "Tender Register",
    { text: "Open Tender Register", hyperlink: "#'Tender Register'!A1" },
  ];
  ...
  sheet.getRow(totalRow).values = [
    "GRAND TOTAL",
    "",
    { formula: `SUM(C13:C${totalRow - 1})` },
    { formula: `SUM(D13:D${totalRow - 1})` },
    { formula: `SUM(E13:E${totalRow - 1})` },
  ];
  sheet.getRow(totalRow).font = { bold: true, color: { argb: NAVY } };
  sheet.getRow(totalRow).fill = {
    type: "pattern",
    pattern: "solid",
  ...
  sheet.getCell(`A${totalRow + 2}`).value = {
    formula: `="This workbook contains "&A5&" registered tenders across "&D5&" clients, with "&B5&" submitted and "&C5&" not yet due."`,
  };
  sheet.getCell(`A${totalRow + 2}`).alignment = { wrapText: true };
  sheet.getCell(`A${totalRow + 2}`).font = {
    italic: true,
    color: { argb: GREY },
  };
  for (let r = totalRow + 3; r <= totalRow + 30; r++)
...
function buildSummary(
  ...
  addTitle(
    sheet,
    "A1:F1",
    "TENDER PORTFOLIO SUMMARY",
    `${orgName} · Live portfolio KPIs, status mix, estimated pipeline, and data-quality indicators.`,
  );
  [24, 20, 20, 22, 18, 18].forEach(
    (width, i) => (sheet.getColumn(i + 1).width = width),
  );
  addKpi(
    sheet,
  ...
  addKpi(
    ...
    "D5",
    "Estimated Pipeline",
    'SUMIFS(TenderRegister[Estimated Value],TenderRegister[Submission Timing],"Not Yet Due")',
    ZAR,
  );
  sheet.getRow(4).height = 28;
  ...
  statuses.forEach((status, i) => {
    ...
    sheet.getRow(r).values = [
      status,
      { formula: `COUNTIF(TenderRegister[Status],A${r})` },
      {
        formula: `SUMIF(TenderRegister[Status],A${r},TenderRegister[Estimated Value])`,
      },
    ];
  ...
  sheet.addTable({
    ...
    ref: `A9:C${9 + statuses.length}`,
    headerRow: true,
    style: { theme: "TableStyleMedium2", showRowStripes: true },
    columns: ["Status", "Tender Count", "Estimated Value"].map((name) => ({
      name,
    })),
    rows: statuses.map((status) => [status, null, null]),
  });
  ...
  sheet.getCell("F9").value = {
    formula: 'COUNTIF(TenderRegister[Data Quality],"*Missing contact*")',
  };
  sheet.getCell("E10").value = "Missing estimates";
  sheet.getCell("F10").value = {
    formula: 'COUNTIF(TenderRegister[Data Quality],"*Missing estimate*")',
  };
  sheet.getCell("E11").value = "Questionable estimates";
  sheet.getCell("F11").value = {
    formula: 'COUNTIF(TenderRegister[Data Quality],"*Questionable estimate*")',
  };

  const findingsRow = Math.max(15, 12 + statuses.length);
  sheet.getCell(`A${findingsRow}`).value = "Key Findings";
  sheet.getCell(`A${findingsRow}`).font = {
    bold: true,
    size: 13,
    color: { argb: NAVY },
  };
  ...
  for (let r = findingsRow + 4; r <= findingsRow + 30; r++)
    sheet.getRow(r).hidden = true;
...
function buildClientSheet(
  ...
  sheet.getCell("A6").value = {
    formula: `FILTER(TenderRegister[[Tender Number]:[Contact Details]],TenderRegister[Client]=$A$2,"")`,
  };
  [24, 72, 24, 48].forEach(
    (width, i) => (sheet.getColumn(i + 1).width = width),
  );
...
function buildClientReport(
  ...
  addTitle(
    ...
    `${orgName} · ${cleanText(clientName)}`,
  );
  [24, 68, 24, 44, 18, 18].forEach(
    (width, i) => (sheet.getColumn(i + 1).width = width),
  );
  ...
  const tableRows = rows.map((row, index) => {
    const r = 9 + index;
    const contact =
      [row.tenderContactName, row.clientEmail, row.clientPhone]
        .filter(Boolean)
        .map(cleanText)
        .join(" | ") || "Not provided";
    return [
      ...
      contact,
      {
        formula: `IF(C${r}="","Not Yet Due",IF(C${r}<=TODAY(),"Submitted","Not Yet Due"))`,
      },
      row.value == null ? null : Number(row.value),
    ];
  });
  sheet.addTable({
    name: "ClientTenderReport",
    ref: `A8:F${Math.max(8 + rows.length, 9)}`,
    headerRow: true,
    style: { theme: "TableStyleMedium2", showRowStripes: true },
    columns: [
      "Tender Number",
