...
export interface StorageCategoryStat {
  category: 'tenders' | 'projects' | 'purchase_orders' | 'extensions' | 'general';
  label: string;
  fileCount: number;
  totalBytes: number;
}
...
export async function getStorageAnalytics(organizationId: string): Promise<{
  ...
  try {
    ...
    const avgFileSizeBytes = totalFiles > 0 ? Math.round(usedBytes / totalFiles) : 0;
    ...
    const categories: Record<StorageCategoryStat['category'], { count: number; bytes: number }> = {
    ...
    const fileTypes: Record<StorageFileTypeStat['fileType'], { count: number; bytes: number }> = {
      pdf: { count: 0, bytes: 0 },
    ...
    const processedDocs: StorageDocumentItem[] = allDocsWithEntities.map((doc) => {
      let entityType: StorageDocumentItem['entityType'] = 'general';
      let entityLabel: string | undefined = undefined;

      if (doc.extensionId) {
        entityType = 'extension';
        entityLabel = doc.tenderNumber ? `Ext on ${doc.tenderNumber}` : 'Extension';
      ...
      } else if (mime.includes('excel') || mime.includes('spreadsheet') || mime.includes('csv')) {
    ...
    const categoryStats: StorageCategoryStat[] = [
      { category: 'tenders', label: 'Tenders', fileCount: categories.tenders.count, totalBytes: categories.tenders.bytes },
      { category: 'projects', label: 'Projects', fileCount: categories.projects.count, totalBytes: categories.projects.bytes },
      { category: 'purchase_orders', label: 'Purchase Orders', fileCount: categories.purchase_orders.count, totalBytes: categories.purchase_orders.bytes },
      { category: 'extensions', label: 'Tender Extensions', fileCount: categories.extensions.count, totalBytes: categories.extensions.bytes },
      { category: 'general', label: 'General / Unlinked', fileCount: categories.general.count, totalBytes: categories.general.bytes },
    ];

    const fileTypeStats: StorageFileTypeStat[] = [
      { fileType: 'pdf', label: 'PDF Documents', fileCount: fileTypes.pdf.count, totalBytes: fileTypes.pdf.bytes },
      { fileType: 'excel', label: 'Excel Spreadsheets', fileCount: fileTypes.excel.count, totalBytes: fileTypes.excel.bytes },
      { fileType: 'word', label: 'Word Documents', fileCount: fileTypes.word.count, totalBytes: fileTypes.word.bytes },
      { fileType: 'image', label: 'Images & Photos', fileCount: fileTypes.image.count, totalBytes: fileTypes.image.bytes },
      { fileType: 'other', label: 'Other Files', fileCount: fileTypes.other.count, totalBytes: fileTypes.other.bytes },
    ];
...
export async function updateOrganizationPlan(
  ...
  try {
    ...
    if (role !== 'owner' && role !== 'admin') {
      return {
        success: false,
        error: 'Only organization owners and administrators can change subscription plans.',
      };
    }
    ...
    if (currentPlan === targetPlan) {
      return { success: true, message: `Your organization is already on the ${targetPlan.toUpperCase()} plan.` };
    }
