...
export async function getProjectStats(organizationId: string) {
  try {
    ...
    for (const p of allProjects) {
      ...
      const hasCriticalRisk = p.risks.some((r) =>
        ["high", "critical"].includes(r.severity),
      );
      const hasOverdueDelivery = p.purchaseOrders.some(
        (po) =>
    ...
    const deliveryStats = {
      ...
      pendingDeliveries: allPOs.filter((po) =>
        ["open", "sent"].includes(po.status),
      ).length,
      partialDeliveries: allPOs.filter(
    ...
    const totalDeliveredValue = parseFloat(
      deliveryValueResult[0]?.totalDelivered || "0",
    );
    ...
    const riskStats = {
      ...
      highCriticalRisks: allActiveRisks.filter((r) =>
        ["high", "critical"].includes(r.severity),
      ).length,
    };
...
export async function addProjectRisk(
  ...
  try {
    ...
    return {
      success: false,
      error: error.message || "Failed to add project risk",
    };
  }
}
...
export async function getProjectRisks(
  ...
  try {
    ...
    return {
      success: false,
      error: error.message || "Failed to fetch project risks",
    };
  }
}
...
export async function updateProjectRiskStatus(
  ...
  try {
    ...
    const riskData = await db
      ...
      .from(projectRisk)
      .where(
        and(eq(projectRisk.id, riskId), eq(projectRisk.projectId, projectId)),
      )
      .limit(1);
    ...
    const updatedRisk = await db
      ...
      .set({
        status,
        mitigationPlan:
          mitigationPlan !== undefined ? mitigationPlan : undefined,
        updatedAt: new Date(),
      })
    ...
    return {
      success: false,
      error: error.message || "Failed to update project risk",
    };
  }
}
...
export async function submitProjectCloseOut(
  ...
  try {
    ...
    return {
      success: false,
      error: error.message || "Failed to close out project",
    };
  }
}
...
export async function getProjectWorkspaceData(
  ...
  try {
    ...
    return {
      success: false,
      error: error.message || "Failed to fetch project workspace data",
    };
  }
}
    ...
    const closeOutCandidates = activeProjects
      .filter((p) => {
        ...
        const allPOsDelivered =
          ...
          p.purchaseOrders.every((po) => po.status === "delivered");
      ...
      .map((p) => ({
        ...
        candidateReason:
          p.contractEndDate && p.contractEndDate < now
            ? "Passed End Date"
            : "All POs Delivered",
      }));
