...
export async function createSupportTicket(input: SupportTicketInput) {
  try {
    ...
    let linkedUserId: string | null =
      sessionUser?.id || validated.userId || null;
    ...
    return {
      success: true,
      ticketId,
      ticketNumber: insertedTicket?.ticketNumber,
      ticketCode,
    };
  } catch (error: any) {
...
export async function emailUserTicketTranscript(ticketId: string) {
  try {
    ...
    return {
      success: false,
      error: err.message || "Failed to email transcript.",
    };
  }
}
 ...
 */
export async function getUnreadTicketCount(): Promise<{
  success: boolean;
  count: number;
}> {
  try {
