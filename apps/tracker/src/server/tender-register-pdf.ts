...
function drawPageNumbers(doc: jsPDF) {
  ...
  for (let i = 1; i <= total; i++) {
    doc.setPage(i);
    doc.text(`Page ${i} of ${total}`, width - MARGIN, height - 5, {
      align: "right",
    });
  }
}
...
function drawTable(
  ...
  const scaled: Column[] = columns.map(([label, columnWidth]) => [
    label,
    columnWidth * scale,
  ]);
  let y = startY;
  ...
  const drawRow = (row: TenderPdfRow, index: number) => {
    const values = includeClient
      ? [
          text(row.tenderNumber).toUpperCase() || "—",
          text(row.clientName) || "—",
          text(row.description) || "—",
          status(row.status),
          date(row.submissionDate),
          date(row.validityDate),
          timing(row.submissionDate),
        ]
      : [
          text(row.tenderNumber).toUpperCase() || "—",
          text(row.description) || "—",
    ...
    const height = Math.max(
      ROW_HEIGHT,
      ...wrapped.map((lines) => lines.length * 3.2 + 3),
    );
    if (y + height > doc.internal.pageSize.getHeight() - 13) {
      ...
      header(
        doc,
        includeClient ? "TENDER REGISTER" : "CLIENT TENDER REPORT",
        includeClient
          ? `${orgName} · Master tender register`
          : `${orgName} · ${text(row.clientName) || "Client"}`,
      );
      y = 34;
      drawHeader();
    }
...
function renderPortfolio(rows: TenderPdfRow[], orgName: string) {
  ...
  header(
    ...
    `${orgName} · Tender submission register`,
  );
  const width = doc.internal.pageSize.getWidth();
  ...
  kpis.forEach(([label, value], i) =>
    kpi(doc, MARGIN + i * (kpiWidth + gap), 35, kpiWidth, label, value),
  );
  ...
  const missingContacts = rows.filter(
    (r) => !contact(r) || contact(r) === "Not provided",
  ).length;
  const missingEstimates = rows.filter(
    (r) => r.value === null || r.value === undefined || r.value === "",
  ).length;
  const questionable = rows.filter(
    (r) =>
      Number(r.value) <= 1 &&
      r.value !== null &&
      r.value !== undefined &&
  ...
  doc.setTextColor(...GREY);
  doc.text(
    `Missing contacts: ${missingContacts}   |   Missing estimates: ${missingEstimates}   |   Questionable estimates: ${questionable}`,
    MARGIN,
    78,
  );
...
async function getRows(organizationId: string, clientId?: string) {
  ...
  return rows.sort((a, b) => {
    const at = a.submissionDate
      ? new Date(a.submissionDate).getTime()
      : -Infinity;
    const bt = b.submissionDate
