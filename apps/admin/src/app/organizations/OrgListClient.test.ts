...
vi.mock("@/components/DataTable", () => ({
  default: vi.fn(),
  PAGE_SIZE: 50,
  getPaginationSlice: (d: unknown[], p: number, s: number) =>
    d.slice((p - 1) * s, p * s),
  getTotalPages: (n: number, s: number) => Math.max(1, Math.ceil(n / s)),
}));
vi.mock("@/components/OrgDrawer", () => ({ default: vi.fn(() => null) }));
vi.mock("@/components/StatusBadge", () => ({ default: vi.fn(() => null) }));
vi.mock("./actions", () => ({
  bulkSuspendOrgs: vi.fn(),
  bulkRestoreOrgs: vi.fn(),
  bulkPurgeOrgs: vi.fn(),
}));
...
const safeDate = fc.date({
  min: new Date(SAFE_MIN_MS),
  max: new Date(SAFE_MAX_MS),
});
