...
import { AdminFeedbackReplySchema } from "@/lib/validations";
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export function FeedbackDetailDrawer({ feedback, open, onOpenChange }: Props) {
  ...
  const handleSendReply = async (e: React.FormEvent) => {
    ...
    const result = AdminFeedbackReplySchema.safeParse({
    ...
    if (res.success) {
      setResult({
        success: true,
        message: res.message || "Response sent successfully!",
      });
      setReplyMessage("");
    } else {
      setResult({
        success: false,
        message: res.error || "Failed to send response.",
      });
    }
  };
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <div className="font-medium text-white flex items-center gap-1.5 mt-0.5">
                  ...
                  <span>
                    {feedback.name || feedback.userName || "Anonymous"}
                  </span>
                </div>
              </div>
              <div>
          ...
          <div className="space-y-3 pt-2">
            ...
            {feedback.email ? (
              ...
              >
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (fieldErrors.subject) {
                        const { subject: _, ...rest } = fieldErrors;
                        setFieldErrors(rest);
                      }
                    }}
                    placeholder={defaultSubject}
                    className={`w-full px-3 py-2 bg-zinc-950 border rounded-lg text-sm text-white focus:outline-none focus:border-amber-500/50 placeholder:text-zinc-600 ${fieldErrors.subject ? "border-red-500" : "border-zinc-800"}`}
                  />
                  {fieldErrors.subject && (
                  ...
                  <textarea
                    ...
                    onChange={(e) => {
                      ...
                      if (fieldErrors.message) {
                        const { message: _, ...rest } = fieldErrors;
                        setFieldErrors(rest);
                      }
                    }}
                    placeholder="Write a response thanking the user or providing an update..."
                    className={`w-full px-3 py-2 bg-zinc-950 border rounded-lg text-sm text-white focus:outline-none focus:border-amber-500/50 placeholder:text-zinc-600 resize-y ${fieldErrors.message ? "border-red-500" : "border-zinc-800"}`}
                  />
                  {fieldErrors.message && (
                    <p className="text-xs text-red-400 mt-1">
                      {fieldErrors.message}
                ...
                <div className="flex items-center justify-between pt-1">
                  <span className="text-[11px] text-zinc-500">
                    Sending to{" "}
                    <strong className="text-zinc-400">{feedback.email}</strong>
                  </span>
                  <button
                    type="submit"
              ...
              <div className="p-4 bg-zinc-900/40 border border-zinc-800 rounded-xl text-xs text-zinc-500 flex items-center gap-2">
                ...
                <span>
                  The submitter did not provide an email address, so an
                  automated email reply cannot be dispatched.
                </span>
              </div>
            )}
          </div>
