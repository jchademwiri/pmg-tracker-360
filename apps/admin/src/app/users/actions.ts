...
import { user, session as sessionTable, member, organization, account } from '@pmg/db/schema';
...
export async function updateUserRole(userId: string, newRole: 'user' | 'admin') {
  ...
  if (currentSession.user.id === userId && newRole !== 'admin') {
    return { success: false, error: 'You cannot remove admin privileges from your own account.' };
  }
  ...
  if (newRole !== 'admin') {
    ...
    if (adminCount <= 1) {
      return { success: false, error: 'Cannot demote the sole system administrator.' };
    }
  }
 ...
 */
export async function updateUserPlan(userId: string, newPlan: 'free' | 'starter' | 'pro') {
  const currentSession = await auth.api.getSession({ headers: await headers() });
  if (!currentSession || (currentSession.user as any).role !== 'admin') {
    return { success: false, error: 'Unauthorized' };
  }
  ...
  try {
    ...
    const e = err as Error;
    return { success: false, error: e.message || 'Failed to update user plan.' };
  }
}
...
export async function toggleUserSuspension(userId: string, suspend: boolean, reason?: string) {
  ...
  try {
    ...
    const e = err as Error;
    return { success: false, error: e.message || 'Failed to change suspension state.' };
  }
}
 ...
 */
export async function bulkUpdateUserRole(userIds: string[], newRole: 'user' | 'admin') {
  const currentSession = await auth.api.getSession({ headers: await headers() });
  if (!currentSession || (currentSession.user as any).role !== 'admin') {
    return { success: false, error: 'Unauthorized' };
  }
  ...
  const filteredIds = userIds.filter((id) => !(id === currentSession.user.id && newRole !== 'admin'));
  ...
  try {
    ...
    revalidatePath('/users');
    return { success: true, message: `Updated role for ${filteredIds.length} user(s).` };
  } catch (err) {
    const e = err as Error;
 ...
 */
export async function bulkUpdateUserPlan(userIds: string[], newPlan: 'free' | 'starter' | 'pro') {
  const currentSession = await auth.api.getSession({ headers: await headers() });
  if (!currentSession || (currentSession.user as any).role !== 'admin') {
    return { success: false, error: 'Unauthorized' };
  }
  ...
  try {
    ...
    revalidatePath('/users');
    return { success: true, message: `Updated plan for ${userIds.length} user(s).` };
  } catch (err) {
    const e = err as Error;
 ...
 */
export async function bulkToggleUserSuspension(userIds: string[], suspend: boolean) {
  const currentSession = await auth.api.getSession({ headers: await headers() });
  if (!currentSession || (currentSession.user as any).role !== 'admin') {
    return { success: false, error: 'Unauthorized' };
  }
  ...
  try {
    if (suspend) {
      await db.delete(sessionTable).where(inArray(sessionTable.userId, safeIds));
    }
    revalidatePath('/users');
    return { success: true, message: `${suspend ? 'Suspended' : 'Restored'} ${safeIds.length} user account(s).` };
  } catch (err) {
    const e = err as Error;
...
export async function bulkDeleteUserAccounts(userIds: string[]) {
  ...
  try {
    ...
    revalidatePath('/users');
    return { success: true, message: `Permanently deleted ${safeIds.length} user account(s).` };
  } catch (err) {
    const e = err as Error;
