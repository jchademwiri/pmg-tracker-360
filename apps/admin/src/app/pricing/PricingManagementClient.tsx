...
export function PricingManagementClient({
  ...
  function handleSavePlan() {
    ...
    const payload: PlanUpdatePayload = {
      ...formData,
      maxTendersPerMonth: isUnlimitedTenders
        ? -1
        : Math.max(1, formData.maxTendersPerMonth),
    };
  ...
  function handleResetDefaults() {
    if (
      !confirm(
        "Are you sure you want to restore all tier pricing and quotas to factory defaults?",
      )
    ) {
      return;
    }
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        setStatusMessage({
          type: "success",
          text: res.message || "Default plans restored",
        });
        window.location.reload();
      } else {
        setStatusMessage({
          type: "error",
          text: res.error || "Failed to reset plans",
        });
      }
    });
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400 mt-0.5">
                Centralized database control for pricing, storage limits,
                monthly tender quotas, and storefront features.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-4 shadow-sm backdrop-blur">
          ...
          <p className="text-2xl font-bold text-white mt-1.5">
            ...
            <span className="text-xs text-zinc-500 font-normal">
              Active / {plans.length} total
            </span>
          </p>
          <p className="text-[11px] text-zinc-500 mt-1">
            Live in client database
          </p>
        </div>
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-4 shadow-sm backdrop-blur">
          ...
          <p className="text-[11px] text-zinc-500 mt-1">
            Quota gates automatically enforce
          </p>
        </div>
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-4 shadow-sm backdrop-blur">
          ...
          <p className="text-[11px] text-zinc-500 mt-1">
            ZAR monthly subscription price
          </p>
        </div>
      </div>
              ...
              <div>
                ...
                <div className="space-y-2.5 py-2 border-y border-zinc-800/80 my-4 text-xs">
                  ...
                  <div className="flex items-center justify-between text-zinc-300">
                    ...
                    <span className="font-semibold text-white">
                      {plan.maxTendersPerMonth === -1
                        ? "Unlimited"
                        : `${plan.maxTendersPerMonth} / month`}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-zinc-300">
                    <span className="flex items-center gap-1.5 text-zinc-400">
                    ...
                    <span className="font-semibold text-white">
                      {plan.maxActiveProjects} projects
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-zinc-300">
                    <span className="flex items-center gap-1.5 text-zinc-400">
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-6 md:p-8 space-y-8">
          <div className="text-center max-w-xl mx-auto space-y-2">
            ...
            <h2 className="text-2xl font-extrabold text-white">
              Transparent, Contractor-First Pricing
            </h2>
            <p className="text-xs text-zinc-400">
              This preview accurately reflects how plans and quotas render to
              users on the home landing page and inside the billing dashboard.
            </p>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-2xl w-full text-white p-6 max-h-[90vh] overflow-y-auto shadow-2xl space-y-6">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
              <div>
                ...
                <p className="text-xs text-zinc-400 mt-0.5">
                  Adjusting these limits updates live database pricing and
                  backend quota gates.
                </p>
              </div>
              <button
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setFormData({
                        ...formData,
                        priceZar: Number(e.target.value),
                      })
                    }
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs text-zinc-300">
                      ...
                      <strong className="text-emerald-400">
                        {formatStorage(formData.maxStorageMb)}
                      </strong>
                    </label>
                    <Input
                      type="number"
                    ...
                    <p className="text-[10px] text-zinc-500">
                      100 MB = Free, 1000 MB = 1GB, 10000 MB = 10GB
                    </p>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      value={
                        isUnlimitedTenders ? "" : formData.maxTendersPerMonth
                      }
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        setFormData({
                          ...formData,
                          maxTendersPerMonth: Number(e.target.value),
                        })
                      }
                      placeholder={
                        isUnlimitedTenders
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        setFormData({
                          ...formData,
                          maxActiveProjects: Number(e.target.value),
                        })
                      }
                      className="bg-zinc-900 border-zinc-700 text-white text-xs h-9 font-mono"
                    />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        setFormData({
                          ...formData,
                          maxOwnedOrgs: Number(e.target.value),
                        })
                      }
                      className="bg-zinc-900 border-zinc-700 text-white text-xs h-9 font-mono"
                    />
