...
export default function BackupsClient() {
  ...
  const [restoreOrg, setRestoreOrg] = useState<{
    key: string;
    orgId: string;
  } | null>(null);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 font-sans select-text">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-zinc-400">
            Create, manage, and restore database backups. Backups are stored in
      ...
      {message && (
        <div
          className={`p-4 rounded-xl flex gap-3 text-sm items-start ${
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-red-200/70 mt-1">
                This will overwrite ALL existing data in the database with the
                data from the selected backup. This action cannot be undone. Are
                you sure?
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-200/70 mt-1">
                Select an organization to restore its data from the backup. Only
                data belonging to the selected organization will be restored.
          ...
          <div className="flex gap-3 items-end">
            <div className="flex-1">
              ...
              <select
                ...
                onChange={(e) =>
                  setRestoreOrg({ ...restoreOrg, orgId: e.target.value })
                }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-3">
          ...
          <p className="text-xs text-zinc-400 leading-relaxed">
            Backups run automatically via cron job. Each backup is a complete
            snapshot of all database tables, compressed and stored securely in
            Cloudflare R2.
          </p>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-3">
          ...
          <p className="text-xs text-zinc-400 leading-relaxed">
            Backups older than 30 days are automatically purged. The system
            keeps a rolling window of backups so you can always restore to a
            recent state.
          </p>
        </div>
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-3">
          ...
          <p className="text-xs text-zinc-400 leading-relaxed">
            Need to recover data for a specific organization? Use the "Org
            Restore" option to selectively restore data scoped to a single
            organization without affecting others.
          </p>
        </div>
      </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-zinc-800">
                      <th className="text-left px-5 py-3.5 text-xs font-semibold text-zinc-400 uppercase tracking-widest">
                        Backup
                      </th>
                      <th className="text-left px-5 py-3.5 text-xs font-semibold text-zinc-400 uppercase tracking-widest">
                        Created (SAST)
                      </th>
                      <th className="text-right px-5 py-3.5 text-xs font-semibold text-zinc-400 uppercase tracking-widest">
                        Size
                      </th>
                      <th className="text-right px-5 py-3.5 text-xs font-semibold text-zinc-400 uppercase tracking-widest">
                        Actions
                  ...
                  <tbody>
                    {(() => {
                      const totalPages = Math.max(
                        1,
                        Math.ceil(backups.length / PAGE_SIZE),
                      );
                            ...
                            <td className="px-5 py-4">
                              <div className="flex items-center gap-3">
                                <div className="h-8 w-8 rounded-lg bg-zinc-800 flex items-center justify-center">
                                  <FileArchive
                                    className={`h-4 w-4 ${isOverallLatest ? "text-amber-400" : "text-zinc-500"}`}
                                  />
                                </div>
                                <div>
