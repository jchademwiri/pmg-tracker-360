...
export async function generatePlatformMasterExcel(): Promise<Buffer> {
  ...
  addMetricRow(
    summarySheet,
    "Soft-Deleted / Inactive Organizations",
    overview.softDeletedTenants,
    "Pending purge / appealed",
  );
  addMetricRow(
    summarySheet,
  ...
  addMetricRow(
    ...
    `${overview.verificationRate}% verification rate`,
  );
  addMetricRow(
    summarySheet,
    "Waitlist Submissions",
    overview.waitlistCount,
    "Prospective leads",
  );
  ...
  addMetricRow(
    summarySheet,
    "Total Storage Consumed (MB)",
    overview.totalStorageMB,
    `${overview.totalStorageGB} GB total`,
  );
  addMetricRow(
    summarySheet,
    "Top Storage Category",
    storageData.categories[0]?.category ?? "N/A",
    `${storageData.categories[0]?.sizeMB ?? 0} MB (${storageData.categories[0]?.percentage ?? 0}%)`,
  );

  summarySheet.addRow([]);
  addSectionHeader(summarySheet, "3. COMMERCIAL PIPELINE (PLATFORM TOTALS)");
  const rPipeline = addMetricRow(
    summarySheet,
    "Total Tender Pipeline Value",
    overview.totalTenderPipelineValue,
    "Cumulative bid sum across all tenants",
  );
  rPipeline.getCell(2).numFmt = '"R "#,##0.00;[Red]-"R "#,##0.00';

  const rAwarded = addMetricRow(
    summarySheet,
    "Total Awarded Tender Value",
    overview.totalTenderAwardedValue,
    "Won contracts value",
  );
  ...
  addMetricRow(
    summarySheet,
    "Total Tenders Managed",
    overview.totalTenders,
    "All bid statuses",
  );
  addMetricRow(
    summarySheet,
  ...
  const rPo = addMetricRow(
    summarySheet,
    "Total Purchase Orders Value",
    overview.totalPOAmount,
    `${overview.totalPOs} total POs logged`,
  );
  ...
  addMetricRow(
    ...
    overview.totalFeedbackCount,
    "Bug reports & features",
  );
  addMetricRow(
    summarySheet,
    "Critical Security Events",
    security.criticalEvents,
    "Audit trail alerts",
  );
  addMetricRow(
    summarySheet,
    "Suspicious Sessions Detected",
    security.suspiciousSessions,
    "Anomalous IP/Device logins",
  );
...
export async function generateStorageAuditExcel(): Promise<Buffer> {
  ...
  addCapRow(
    "Cloudflare Plan Tier Limit",
    `${overview.totalStorageCapacityGB ?? 10}.00 GB`,
    "Cloudflare R2 Monthly Free Tier",
  );
  addCapRow(
    "Total Combined Storage Used",
    `${overview.totalStorageMB?.toLocaleString() ?? 0} MB`,
    `${overview.totalStorageGB ?? 0} GB across all detected buckets`,
  );
  addCapRow(
    "Available Free Storage Remaining",
    `${overview.availableStorageGB ?? 10} GB`,
    `${overview.availableStorageMB?.toLocaleString() ?? 0} MB remaining before pay-as-you-go`,
  );
  addCapRow(
    "Capacity Utilization Rate",
    `${overview.storageUtilizationPct ?? 0}%`,
    (overview.storageWarningStatus ?? "normal").toUpperCase(),
  );
  addCapRow(
    "Total Document Attachments",
    overview.totalDocuments ?? 0,
    "Files tracked in PMG Tracker 360 database",
  );
  addCapRow(
    "Live Cloudflare API Connection",
    storageData.storageOverview?.connectedLiveApi
      ? "Connected (Multi-Bucket Sync)"
      : "Local DB Monitored",
    "Set CLOUDFLARE_API_TOKEN for live multi-app discovery",
  );
