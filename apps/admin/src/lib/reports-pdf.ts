...
export async function generatePlatformExecutivePdf(): Promise<Buffer> {
  ...
  const addHeader = (tabTitle: string, subTitle: string) => {
    ...
    doc.text(
      `— ${subTitle}`,
      margin + doc.getTextWidth(tabTitle.toUpperCase()) + 2,
      17,
    );
    ...
    doc.text(`Generated: ${dateStr}`, pageWidth - margin, 17, {
      align: "right",
    });
  };
  ...
  drawTableRow(
    ...
    `${overview.totalTenants} organizations`,
    `${overview.activeTenants} active, ${overview.softDeletedTenants} archived/soft-deleted`,
  );
  drawTableRow(
    "Total Platform Users",
    `${overview.totalUsers} accounts`,
    `${overview.verifiedUsers} verified (${overview.verificationRate}% verification rate)`,
  );
  drawTableRow(
    "Total Awarded Tender Sum",
    fmtCurrency(overview.totalTenderAwardedValue),
    "Cumulative contract value awarded from won tenders",
  );
  drawTableRow(
    "Active Awarded Projects",
    `${overview.activeProjects} active projects`,
    `${overview.totalProjects} lifetime projects logged in system`,
  );
  drawTableRow(
    "Purchase Orders Committed",
    `${overview.totalPOs} POs (${fmtCurrency(overview.totalPOAmount)})`,
    "Total financial commitment across suppliers",
  );
  drawTableRow(
    "Open Support Tickets",
    `${overview.openSupportTickets} tickets open`,
    "Support inquiries awaiting resolution",
  );
  drawTableRow(
    "User Feedback Submissions",
    `${overview.totalFeedbackCount} submissions`,
    "User-submitted bug reports & feature requests",
  );
  drawTableRow(
    "Waitlist Prospective Leads",
    `${overview.waitlistCount} prospective companies`,
    "Leads registered on waitlist",
  );
  ...
  doc.setTextColor(100, 116, 139);
  doc.text(
    `Total: ${allTenants.length} registered organizations`,
    pageWidth - margin,
    y,
    { align: "right" },
  );
  ...
  const drawTenantTableHeader = () => {
    ...
    doc.text("Storage (MB)", pageWidth - margin - 3, y + 4.5, {
      align: "right",
    });
    y += 6.5;
  };
  ...
  allTenants.forEach((tenant, idx) => {
    ...
    doc.setTextColor(15, 23, 42);
    const nameStr =
      tenant.name.length > 28 ? tenant.name.slice(0, 26) + "…" : tenant.name;
    ...
    doc.text(tenant.memberCount.toString(), margin + 110, y + 4.2, {
      align: "right",
    });
    doc.text(tenant.tenderCount.toString(), margin + 130, y + 4.2, {
      align: "right",
    });
    doc.text(tenant.projectCount.toString(), margin + 150, y + 4.2, {
      align: "right",
    });
    doc.text(
      `${tenant.storageMB.toLocaleString()} MB`,
      pageWidth - margin - 3,
      y + 4.2,
      { align: "right" },
    );
  ...
  storageData.categories.forEach((cat, idx) => {
    ...
    doc.text(cat.count.toLocaleString(), margin + 80, y + 4.2, {
      align: "right",
    });
    doc.text(`${cat.sizeMB.toLocaleString()} MB`, margin + 125, y + 4.2, {
      align: "right",
    });
    doc.text(`${cat.percentage}%`, pageWidth - margin - 3, y + 4.2, {
      align: "right",
    });
  ...
  storageData.topTenants.forEach((tenant, idx) => {
    ...
    doc.setTextColor(15, 23, 42);
    const nameStr =
      tenant.name.length > 32 ? tenant.name.slice(0, 30) + "…" : tenant.name;
    doc.text(nameStr, margin + 3, y + 4.2);

    doc.setFont("helvetica", "normal");
    doc.setTextColor(71, 85, 105);
    doc.text(tenant.tenderCount.toLocaleString(), margin + 80, y + 4.2, {
      align: "right",
    });
    doc.text(tenant.projectCount.toLocaleString(), margin + 105, y + 4.2, {
      align: "right",
    });
    doc.text(tenant.documentCount.toLocaleString(), margin + 130, y + 4.2, {
      align: "right",
    });
    doc.text(
      `${tenant.storageMB.toLocaleString()} MB`,
  ...
  const drawSecHeader = () => {
    ...
    doc.text("Date & Time", pageWidth - margin - 3, y + 4.5, {
      align: "right",
    });
    y += 6.5;
  };
  ...
  if (security.recentCriticalLogs.length === 0) {
    ...
    doc.setTextColor(100, 116, 139);
    doc.text(
      "No critical security alerts currently recorded.",
      margin + 3,
      y + 5.5,
    );
    y += 8;
    ...
    security.recentCriticalLogs.forEach((log, idx) => {
      if (y > pageHeight - 20) {
        ...
        addHeader(
          ...
          "Critical Audit Events (Continued)",
        );
        y = 34;
        drawSecHeader();
      }
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(
      "Confidential — For Platform System Owners Only",
      margin,
      pageHeight - 5.5,
    );
    doc.text(
...
export async function generateStorageAuditPdf(): Promise<Buffer> {
  ...
  const addHeader = (tabTitle: string, subTitle: string) => {
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(203, 213, 225);
    doc.text(`Generated: ${dateStr}`, pageWidth - margin, 11, {
      align: "right",
    });
  ...
  doc.setTextColor(15, 23, 42);
  doc.text(
    "1. Cloudflare R2 Storage Overview & Multi-Bucket Capacity",
    margin,
    y,
  );
  y += 5;
  ...
  doc.text(
    `Provisioned Plan Capacity: ${overview.totalStorageCapacityGB ?? 10}.00 GB Cloudflare Free Tier | Available Remaining: ${overview.availableStorageGB ?? 10} GB (${overview.availableStorageMB?.toLocaleString() ?? 0} MB)`,
    margin + 4,
    y + 11.5,
  );
  doc.setFont("helvetica", "bold");
  doc.text(
    `Capacity Utilization: ${overview.storageUtilizationPct ?? 0}% (${(overview.storageWarningStatus ?? "normal").toUpperCase()}) | Detected Buckets: ${storageData.storageOverview?.buckets?.length ?? 0}`,
    margin + 4,
    y + 16.5,
  );
  y += 26;
  ...
  storageData.categories.forEach((cat, idx) => {
    ...
    doc.text(`${cat.sizeMB.toLocaleString()} MB`, margin + 120, y + 4.2, {
      align: "right",
    });
    doc.text(`${cat.percentage}%`, pageWidth - margin - 3, y + 4.2, {
      align: "right",
  ...
  allTenants.forEach((tenant, idx) => {
    if (y > pageHeight - 20) {
      doc.addPage();
      addHeader(
        "Tab 2: Tenant Storage Quotas",
        "Consumption Schedule (Continued)",
      );
      y = 34;
      drawStorageOrgHeader();
    }
    ...
    doc.text(tenant.documentCount.toLocaleString(), margin + 100, y + 4.2, {
      align: "right",
    });
    doc.text(
      `${tenant.storageMB.toLocaleString()} MB`,
...
export async function generateSecurityAuditPdf(): Promise<Buffer> {
  ...
  doc.setTextColor(30, 41, 59);
  doc.text(
    `• Total Audit Trail Entries: ${security.totalLogs.toLocaleString()}`,
    margin + 4,
    y + 5.5,
  );
  doc.text(
  ...
  doc.text(
    `• Suspicious Active Sessions: ${security.suspiciousSessions} flagged of ${security.totalSessions} sessions`,
    margin + 4,
    y + 16.5,
  );
