...
export async function getCloudflareR2StorageStats(
  ...
  try {
    ...
    const bucketPromises = rawBuckets.map(async (b) => {
      const isCurrent =
        b.name.toLowerCase() === currentBucketName.toLowerCase();
      try {
        ...
        if (usageRes.ok) {
          ...
          const objectCount = Number(
            payload.objectCount || payload.objects || 0,
          );

          return {
            name: b.name,
            isCurrentApp: isCurrent,
            objectCount: isCurrent
              ? Math.max(objectCount, pmgDocCount)
              : objectCount,
            sizeBytes: isCurrent ? Math.max(sizeBytes, pmgDbBytes) : sizeBytes,
            sizeMB: Number(
              (
                (isCurrent ? Math.max(sizeBytes, pmgDbBytes) : sizeBytes) /
                (1024 * 1024)
              ).toFixed(2),
            ),
            sizeGB: Number(
    ...
    const totalUsedBytes = bucketResults.reduce(
      (acc, b) => acc + b.sizeBytes,
      0,
    );
    const totalUsedMB = Number((totalUsedBytes / (1024 * 1024)).toFixed(2));
    const totalUsedGB = Number(
      (totalUsedBytes / (1024 * 1024 * 1024)).toFixed(3),
    );
    ...
    const otherAppsStorageMB = Math.max(
      0,
      Number((totalUsedMB - pmgStorageMB).toFixed(2)),
    );
    const otherAppsStorageGB = Math.max(
      0,
      Number((totalUsedGB - pmgStorageGB).toFixed(3)),
    );
