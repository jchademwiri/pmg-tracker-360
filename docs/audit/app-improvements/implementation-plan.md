...
|---|------|-------|:------:|-------|
| 1.1 | **Fix `won` → `awarded` status mismatch** | `getAvailableTendersForProjects` queries for `status = 'won'` but schema uses `'awarded'` | S | `apps/tracker/src/server/tenders.ts` |
| 1.2 | **Re-enable document upload** | Upload shows "currently unavailable" despite server support existing | M | `apps/tracker/src/components/tenders/tender-form.tsx`, `tender-details.tsx` |
| 1.3 | **Create `tender_follow_up` table** | Follow-ups UI exists but data is not persisted — lost on refresh | M | `packages/db/src/schema.ts`, migration |
| 1.4 | **Expand PO status enum** | Only `open/sent/delivered` — no draft, partially_delivered, completed, cancelled | S | `packages/db/src/schema.ts` |
| 1.5 | **Add `priority` field to tender** | No way to prioritise tenders | S | `packages/db/src/schema.ts` |
| 1.6 | **Create shared `StatusBadge` component** | Status colours inconsistent across all components | S | `packages/ui/src/components/ui/status-badge.tsx` |
...
| # | Task | Issue | Effort | Files |
|---|------|-------|:------:|-------|
| 2.1 | **Add dashboard action queue band** | No urgency signals — users must scroll to find critical items | M | `apps/tracker/src/app/(dashboard)/dashboard/` |
| 2.2 | **Add project/PO risk cards** | Delivery health invisible on dashboard | M | Dashboard components |
| 2.3 | **Add nav badge counts** | No urgency signalling in sidebar | S | `apps/tracker/src/components/shared/navigation/app-sidebar.tsx` |
| 2.4 | **Rename navigation groups** | "Procurement Cycle" confusing; "Tender Pipeline" → "Tender Management" | S | `apps/tracker/src/data/dashboad-links.ts` |
| 2.5 | **Add direct workflow links** | Follow-ups, submitted/awarded tenders, deliveries, overdue items not exposed | M | Sidebar + dashboard |
| 2.6 | **Add KPI trend indicators** | No context for whether metrics improving or declining | S | Dashboard metric cards |
| 2.7 | **Add urgency alert banner** | Missing deadlines/overdue items not surfaced at top | S | Dashboard layout |
...
| # | Task | Issue | Effort | Files |
|---|------|-------|:------:|-------|
| 3.1 | **Add tender stages/preparation workflow** | No review/approval/preparation states | L | Schema + tender server actions |
| 3.2 | **Persist follow-ups + next follow-up date** | Follow-up data lost on refresh (schema from Phase 1) | M | Tender server actions + UI |
| 3.3 | **Add result capture dialog/form** | Cannot capture award amount, loss reason, or evaluation notes | M | `tender-details.tsx` |
| 3.4 | **Add tender checklist tied to documents** | Compliance checklist disconnected from uploads | M | Tender detail |
| 3.5 | **Add tender mobile cards** | Tender register unusable on mobile | L | `tenders-table.tsx` |
| 3.6 | **Add filtered tender queues** | No way to see "closing soon", "awaiting result", "submitted" | M | Tender overview |
| 3.7 | **Convert tender form to multi-step** | 15+ fields on one page — overwhelming | L | `tender-form.tsx` |
| 3.8 | **Add draft saving** | Complex forms must be completed in one sitting | M | All forms |
...
| # | Task | Issue | Effort | Files |
|---|------|-------|:------:|-------|
| 4.1 | **Add POs to project detail** | Project page doesn't show linked POs | M | `projects/[id]/page.tsx` |
| 4.2 | **Add contract/linked tender summaries** | No context about originating tender | M | Project detail |
| 4.3 | **Add project delivery/risk overview** | Delivery health invisible in project context | M | Project detail |
| 4.4 | **Add activity timeline** | No activity log across lifecycle events | L | Project detail + schema |
| 4.5 | **Add project progress tracking** | No way to see project completion % | M | Project register + detail |
| 4.6 | **Add project close-out workflow** | Projects linger as "active" indefinitely | M | Project server actions |
| 4.7 | **Add awaiting-PO and delayed-project queues** | Managers can't see project health | M | Dashboard |
...
| # | Task | Issue | Effort | Files |
|---|------|-------|:------:|-------|
| 5.1 | **PO line-item editor + server CRUD** | Schema exists but no UI or API | L | `po-form.tsx`, `purchase-orders.ts` |
| 5.2 | **Delivery note capture with POD upload** | No way to record deliveries | L | New delivery components |
| 5.3 | **Validate delivered vs outstanding quantities** | No guardrails against over-delivery | M | Server actions |
| 5.4 | **Auto-calculate PO completion status** | Manual status updates only | M | Server actions |
| 5.5 | **Mobile delivery capture flow** | Field supervisors can't record deliveries on phone | L | New mobile components |
| 5.6 | **Expand PO form to include line items** | PO creation is header-only | M | `po-form.tsx` |
...
| # | Task | Issue | Effort | Files |
|---|------|-------|:------:|-------|
| 6.1 | **Shared mobile card component** | Tables overflow on mobile | M | `packages/ui/src/components/ui/mobile-card.tsx` |
| 6.2 | **Mobile filter drawer/bottom sheet** | Desktop search filters don't work on mobile | M | Filter components |
| 6.3 | **Sticky mobile action bars** | No quick actions on mobile detail pages | S | Detail page layouts |
| 6.4 | **Mobile bottom navigation** | Sidebar collapses — no mobile nav pattern | M | App layout |
| 6.5 | **Convert remaining forms to steppers** | Complex forms still single-page on mobile | L | Tender/PO forms |
| 6.6 | **Add draft/autosave to all forms** | Form abandonment risk | M | Form infrastructure |
| 6.7 | **Pull-to-refresh on mobile registers** | No mobile refresh pattern | S | Register components |
...
| # | Task | Issue | Effort | Files |
|---|------|-------|:------:|-------|
| 7.1 | **Activity/event table** | No reliable timeline — inferred from `updatedAt` | M | Schema + server |
| 7.2 | **Notification triggers** | No email/alert triggers at milestones | L | Notification service |
| 7.3 | **Command palette (Cmd+K)** | Power users can't navigate quickly | M | New component |
| 7.4 | **Reporting dashboard** | No win/loss, delivery performance, overdue POs reports | L | Reports pages |
| 7.5 | **Premium UI polish** | Card elevation, transitions, micro-interactions | M | CSS + components |
| 7.6 | **Export functionality** | No data export on any register | M | Register components |
| 7.7 | **Keyboard shortcuts** | No power-user keyboard navigation | M | Layout + shortcuts |
...
1. **Fix `won` → `awarded`** — 5 min fix, unblocks project creation
2. **Expand PO status enum** — 30 min, enables lifecycle tracking
3. **Add `priority` to tenders** — 30 min, enables prioritisation
4. **Create `StatusBadge` component** — 1 day, consistent status display everywhere
5. **Add nav badge counts** — 1 day, immediate urgency signalling
6. **Rename confusing nav labels** — 10 min, reduces cognitive load
7. **Add KPI trend arrows** — 2 hours, gives context to metrics
8. **Add urgency alert banner** — 2 hours, surfaces critical items
