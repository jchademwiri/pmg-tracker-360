...
- **Verify:** Every filter status drawn from `TENDER_LIFECYCLE` shown in the tender list filter also appears, with the same label, in the tender detail lifecycle stepper. This rule does not apply to `lost`/`closed`/`cancelled` (terminal statuses added to the filter per resolution #2, but outside `TENDER_LIFECYCLE`'s progression) or to `closing_soon`/`under_preparation`/`awaiting_results` (derived query predicates per resolution #1, not stored statuses) — decide separately whether the stepper should gain terminal-state handling, or whether the filter's terminal values simply have no stepper equivalent by design. Separately, confirm the three derived quick-view filters still function correctly as predicates over lifecycle stage + deadline math, not as literal status-string comparisons.
...
- **Consumers to migrate:**
  - `tenders-table.tsx` + `tenders-search-filters.tsx` (Tender Register, `/tenders`) — currently uses component state (`internalFilters`), not URL sync. Note `tenders/overview/client-wrapper.tsx` (`/tenders/overview`) already implements the URL-sync contract via `router.replace` for `search`/`status`/`clientId`/`sortBy`/`sortOrder`/`page` — this task is extracting that existing contract into a shared hook and applying it to the Register page too, not building URL sync from scratch.
