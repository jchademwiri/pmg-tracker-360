...
2. **Tender status model mismatch persists.** `tenders-search-filters.tsx`'s status filter is an 8-value set: 5 persisted statuses (`open`, `closed`, `evaluation`, `awarded`, `lost`) plus 3 synthetic quick-view buckets that aren't stored statuses at all (`closing_soon`, `under_preparation`, `awaiting_results`). `status-badge.tsx`'s `TENDER_LIFECYCLE` defines a separate 8-stage ordered progression (`new`, `review`, `approved_to_prepare`, `preparation`, `ready`, `submitted`, `evaluation`, `awarded`) used in `tender-details.tsx`. A tender's list-page filter and its detail-page lifecycle stepper are describing two different state machines — `open` in the filter has no single equivalent lifecycle stage. This is exactly the mismatch the original audit flagged as high priority, and it's unchanged. See `10-implementation-plan-2026-07.md` §1.2 for the resolution options.
