...
This document converts the PDF report generator feature into an implementation sequence. It defines the recommended phases, tasks, validation points, and expected deliverables before merging the feature.
...
- Create `apps/admin/src/server/report-data/platform-overview-data.ts`.
- Create `packages/reports/src/admin/platform-overview-report.ts`.
...
- Create `apps/admin/src/server/report-data/organization-usage-data.ts`.
- Create `packages/reports/src/admin/organization-usage-report.ts`.
...
- Add unit tests for filename sanitisation.
- Add unit tests for currency/date formatters.
- Add unit tests for empty table rendering.
- Add integration checks for route authorization.
- Add manual QA for long tables and multi-page PDFs.
