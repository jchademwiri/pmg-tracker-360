...
This document defines the proposed technical design for adding PDF report generation to PMG Tracker 360. It covers the architecture, module boundaries, API routes, rendering strategy, authorization, report data flow, error handling, and implementation approach.
...
This layer should live inside the relevant app because each app already owns its own auth checks and server queries.
...
Use consistent sanitized filenames.

```txt
Tender-Register-2026-06-25.pdf
Project-Status-PRJ-001-2026-06-25.pdf
Purchase-Order-PO-001.pdf
Platform-Overview-2026-06-25.pdf
Organization-Usage-Acme-Projects-2026-06-25.pdf
```
...
Use summary cards for high-level metrics.
...
- Page number.
- Generated timestamp.
- Optional system label, e.g. `Generated by PMG Tracker 360`.
...
The UI should expose report downloads from:
...
- Avoid browser-based PDF generation in serverless environments unless strictly required.
- Do not write PDF files to disk for normal downloads.
- Generate the buffer in memory and return it directly.
- Watch bundle size if the shared report package grows.
