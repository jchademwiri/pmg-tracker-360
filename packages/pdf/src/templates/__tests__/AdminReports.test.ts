...
describe("AdminReportPdf Template", () => {
  it("renders a Platform Executive report successfully", async () => {
    const mockModel: AdminReportPdfModel = {
      ...
      kpiCards: [
        ...
        },
        {
          ...
          value: "R 145,000,000.00",
          subtext: "128 total bids",
          variant: "success",
        },
        {
          label: "Platform S3 Storage",
          value: "2,450 MB",
          subtext: "512 uploaded files",
          variant: "default",
        },
      ],
      sections: [
            ...
            rows: [
              {
                ...
                value: "45 orgs",
                context: "42 active, 3 archived",
              },
              {
                metric: "Active Awarded Projects",
                value: "88 projects",
                context: "Lifetime contracts",
              },
            ],
          },
        },
    ...
    const result = await renderToPdf(
      React.createElement(AdminReportPdf, { data: mockModel }),
    );
  ...
  it("renders a Storage Audit report with callouts and quotas", async () => {
      ...
      kpiCards: [
        {
          ...
          value: "1,240 MB",
          subtext: "1.24 GB consumed",
          variant: "primary",
        },
        {
          label: "Quota Limit",
          value: "10.00 GB",
          subtext: "Cloudflare Free Tier",
          variant: "default",
        },
        {
          label: "Utilization Rate",
          value: "12.4%",
          subtext: "Healthy capacity",
          variant: "success",
        },
      ],
      sections: [
        {
          ...
          table: {
            ...
            rows: [
              {
                category: "Tender Submissions",
                count: "140",
                size: "892 MB",
                share: "72.0%",
              },
              {
                category: "Purchase Order Attachments",
                count: "85",
                size: "220 MB",
                share: "17.7%",
              },
    ...
    const result = await renderToPdf(
      React.createElement(AdminReportPdf, { data: mockStorageModel }),
    );
    const pdfBuffer = Buffer.from(result.bytes);
    expect(pdfBuffer).toBeInstanceOf(Buffer);
    expect(pdfBuffer.length).toBeGreaterThan(1000);
  ...
  it("renders a Security Audit report with critical event alerts", async () => {
    const mockSecurityModel: AdminReportPdfModel = {
      ...
      kpiCards: [
        ...
        },
        {
          ...
          value: "1",
          subtext: "Flagged anomalous login",
          variant: "warning",
        },
      ],
      sections: [
        {
          ...
          table: {
            ...
            rows: [
              {
                action: "auth.failed_brute_force",
                resource: "user_session",
                severity: "critical",
                ip: "197.229.4.12",
                dateTime: "13 Sep 2026, 08:30",
              },
              {
                action: "org.role_escalation",
                resource: "organization",
                severity: "warning",
                ip: "102.165.32.1",
                dateTime: "12 Sep 2026, 14:15",
              },
            ],
          },
        },
    ...
    const result = await renderToPdf(
      React.createElement(AdminReportPdf, { data: mockSecurityModel }),
    );
    const pdfBuffer = Buffer.from(result.bytes);
    expect(pdfBuffer).toBeInstanceOf(Buffer);
    expect(pdfBuffer.length).toBeGreaterThan(1000);
