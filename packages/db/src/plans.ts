...
export async function getSubscriptionPlans(
  ...
  try {
    ...
    if (!plans || plans.length === 0) {
      return DEFAULT_SUBSCRIPTION_PLANS.filter(
    ...
    console.warn(
      "[getSubscriptionPlans] Database query failed, using in-memory plan definitions:",
      err,
    );
    return DEFAULT_SUBSCRIPTION_PLANS.filter(
      (p) => includeInactive || p.isActive,
...
export async function getSubscriptionPlan(
  ...
  const defaultPlan = DEFAULT_SUBSCRIPTION_PLANS.find(
    (p) => p.id === normalizedId,
  );
...
export async function getPlanLimits(
  ...
  return {
    ...
    maxTendersPerMonth:
      plan.maxTendersPerMonth === -1 ? Infinity : plan.maxTendersPerMonth,
